...
func watchForChanges(clientset *kubernetes.Clientset, snapshotCache *cache.SnapshotCache, configmapKey, configmap *string, clustersResource, routesResource *[]types.Resource, mutex *sync.Mutex) {
	for {
		...
		l.Debugf("Listing existing K8S configmaps in the namespace: %s", namespace)
		// All Namespaces???? Need Permissions ---> corev1.NamespaceAll
...
func updateRoutesConfig(routesResource *[]types.Resource, index int, configMap *corev1.ConfigMap) error {
	var err error
		...
		if open {
			...
			case watch.Modified:
				...
				if configMap, ok := event.Object.(*corev1.ConfigMap); ok {
					...
					l.Debugf("ConfigMap Name: %s", configmapName)
					if strings.Contains(configmapName, "-envoy-") {
						//configmapNamespace := configMap.Namespace
						var err error
						if strings.Contains(configmapName, "-envoy-main-routes-config") {
							l.Debugf("*** Updating Main Routes ***")
							err = updateRoutesConfig(routesResource, 0, configMap)
						}
						if strings.Contains(configmapName, "-envoy-ext-authz-routes-config") {
							l.Debugf("*** Updating ExtAuthZ Routes ***")
							err = updateRoutesConfig(routesResource, 1, configMap)
						}
						if strings.Contains(configmapName, "-envoy-pl-main-routes-config") {
							l.Debugf("*** Updating Private-Link Main Routes ***")
							err = updateRoutesConfig(routesResource, 2, configMap)
						}
						...
						if err == nil {
							newSnapshot, _ := cache.NewSnapshot(
								time.Now().Format("2006-01-02-15-04-05"),
								map[rsrc.Type][]types.Resource{
									rsrc.ClusterType: *clustersResource,
									rsrc.RouteType:   *routesResource,
								},
							)
						...
						l.Debugf("Skipping Non-Envoy ConfigMap...")
					}
				}
				mutex.Unlock()
			case watch.Deleted:
...
func setupWatcher(clientset *kubernetes.Clientset, snapshotCache *cache.SnapshotCache) {
	...
	// Initialize
	routesResource = make([]types.Resource, 3)
