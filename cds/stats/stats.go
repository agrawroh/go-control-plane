...
func getCountersByRouteTable(routeTableName string) RdsStatsByRouteTable {
	if _, exists := countersByRouteTable[routeTableName]; !exists {
		countersByRouteTable[routeTableName] = RdsStatsByRouteTable{
			totalVirtualClustersCount: makeCounterWithRouteTableNameTagFunc("cluster_total_count", routeTableName),
			createErrorCount:          makeCounterWithRouteTableNameTagFunc("cluster_create_error_count", routeTableName),
			createSuccessCount:        makeCounterWithRouteTableNameTagFunc("cluster_create_success_count", routeTableName),
		}
	}
